...
class PyTorchModel(ModelInterface):
    ...
    def predict(self, x: np.ndarray, grad: bool = False, **kwargs) -> np.array:
        ...
        with grad_context:
            if "question" in model_predict_kwargs.keys():
                question = model_predict_kwargs['question']
                question_length = model_predict_kwargs['q_length']
                model_predict_kwargs_copied = {key: item for key, item in model_predict_kwargs.items() if key not in ["question", "q_length"]}
                pred = self.model(torch.Tensor(x).to(self.device), 
                                  torch.Tensor(question).long().to(self.device),
                                  question_length, **model_predict_kwargs_copied)
            else:
