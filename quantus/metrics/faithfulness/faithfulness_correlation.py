...
class FaithfulnessCorrelation(PerturbationMetric):
    ...
    def evaluate_instance(
        ...
        model_predict_kwargs = self.model_predict_kwargs or dict()
        model_predict_kwargs_copied = {key: item for key, item in model_predict_kwargs.items()} 
        if "question" in self.explain_func_kwargs.keys():
            model_predict_kwargs_copied['question'] = np.expand_dims(self.explain_func_kwargs['question'][i], 0)
            model_predict_kwargs_copied['q_length'] = np.expand_dims(self.explain_func_kwargs['q_length'][i], 0)
        # Predict on input.
        x_input = model.shape_input(x, x.shape, channel_first=True)
        y_pred = float(model.predict(x_input, **model_predict_kwargs_copied)[:, y])
